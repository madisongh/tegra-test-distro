...
PACKAGES += "${@bb.utils.contains('PACKAGECONFIG', 'graphical-examples', '${PN}-graphical-examples', '', d)}"

PACKAGECONFIG ??= "${@bb.utils.contains('DISTRO_FEATURES', 'x11 opengl', 'glsl graphical-examples', '', d)}"
PACKAGECONFIG[rsusb] = "-DFORCE_RSUSB_BACKEND:BOOL=ON,-DFORCE_RSUSB_BACKEND:BOOL=OFF"
PACKAGECONFIG[glsl] = "-DBUILD_GLSL_EXTENSIONS:BOOL=ON,-DBUILD_GLSL_EXTENSIONS:BOOL=OFF,virtual/libgl libpng libglu glfw gtk+3"
PACKAGECONFIG[graphical-examples] = "-DBUILD_GRAPHICAL_EXAMPLES:BOOL=ON,-DBUILD_GRAPHICAL_EXAMPLES:BOOL=OFF"
